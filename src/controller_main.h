...
/**
 * @brief Commands received from desktop (or sent to the desktop) in status command
 * @details The command is sent by both the device and the desktop to indicate corresponding
 * state of the flow from context of their respective context.
 *
 * @see READY_STATE_PACKET, mark_device_state(), is_device_ready(), \_success_listener(), \_abort\_(),
 ...
 * @details Temporarily store user input in the struct members. The possible inputs are
 * text/character inputs and choice in the list (for menu/submenu options, back/next options,
 * selection from list of choices).
 *
 * @see Flow_level
 * @since v1.0.0
 *
 * @note When handling sensitive information (such as PIN or mnemonics) as input, care is taken
 * that this structure is cleared out just after its use. As a result of this strategy, back
 * controllers will not have access to same information.
 */
#pragma pack(push, 1)
 ...
 * @details This is the core application-flow state variable for the state machine based
 * architecture of the application. It enables simple navigation/jumps through flows.
 *
 * @see Screen_input, level_one_tasks(), level_one_tasks_initial(), level_one_controller(), level_one_controller_initial(),
 * level_one_controller_b(), controller_level_four.h, tasks_level_four.h, controller_tap_cards.h, tasks_tap_cards.h,
 * controller_add_wallet.h, tasks_add_wallet.h, controller_old_wallet.h, tasks_old_wallet.h, controller_advanced_settings.h,
 * tasks_advanced_settings.h, tasks.h
 * @since v1.0.0
 *
 * @note
 */
...
typedef struct Flow_level {
  ...
  /**
     ...
     * etc.)
     *
     * @see Screen_input, level_three_old_wallet_tasks(), level_three_old_wallet_controller(), level_three_old_wallet_controller_b(),
     * LEVEL_THREE_NEW_WALLET_TASKS, LEVEL_THREE_OLD_WALLET_TASKS, LEVEL_THREE_ADVANCED_SETTINGS_TASKS, GENERATE_WALLET_TASKS,
  ...
  /**
     ...
     * verify wallet, unlock wallet, device_provision (only in initial application).
     *
     * @see SYNC_CARDS_TASKS, VERIFY_WALLET_TASKS, WALLET_LOCKED_TASKS, START_DEVICE_AUTHENTICATION, SEND_TRANSACTION_ETH,
     * RECEIVE_TRANSACTION_TASKS_ETH, RECEIVE_TRANSACTION_TASKS, SEND_TRANSACTION, ADD_COINS_TASKS, EXPORT_WALLET_TASKS,
 ...
 * @details The transition happens based on checks on Counter.level and Counter.next_event_flag.
 * The Counter.next_event_flag governs if there are any events to be handled which initiates the chain of call to tasks
 * from level_one_tasks() or level_one_tasks_initial(). Each task and controller must check the Counter.level to determine
 * if the state is at higher level.
 *
 ...
 * to update the flow_level and counters for the next event. The function acts as bridge between lvgl registered UI
 * event handlers and application controllers to enable post processing in the application context as the user progresses
 * in a particular process/flow.
 *
 * @see Counter.next_event_flag, level_one_controller(), level_one_controller_initial(), next_controller
 * @since v1.0.0
 */
void mark_event_over();

/**
 * @brief On user confirmation, assigns the value of choice made by user to the flow_level.screen_input.list_choice.
 * @details This function is called by the lvgl UI components to handover the user input choice to the application.
 * The function copies the provided input from UI component to the Flow_level.screen_input.list_choice.
 *
 ...
 * @details This function sets Counter.next_event_flag to true and makes calls to the back controllers
 * to update the flow_level and counters for the previous event. The function acts as bridge between lvgl registered UI
 * event handlers and the application back controllers to enable post processing in the application context as the user
 * tries to exit or cancel the process/flow.
 *
 ...
 * @details This functions sets the 1 to all flow_level levels and sets Counter.level to LEVEL_ONE.
 * Sets the Counter.next_event_flag to true and makes the device state as ready. The Wallet.password_double_hash
 * is cleared and a call cy_free() releases all the allocated memory on heap using cy_malloc().
 *
 * @see counter, flow_level, Wallet.password_double_hash, mark_device_state(), cy_free(), cy_malloc()
 * @since v1.0.0
 */
...
/**
 * @brief Resets the screen input choice. Sets the Flow_level.screen_input.list_choice to 0.
 *
 ...
 * @details Sets the Flow_level.show_error_screen to true and sets error message string to Flow_level.error_screen_text.
 * This is error message is shown to the user in the next event loop iteration.
 *
 ...
 * @details The function internally calls snprintf to copy the input data to application's temporary
 * input buffer for processing in subsequent event loops.
 *
 ...
 * @details The choice is stored in the flow_level.screen_input.expected_list_choice for future reference in the
 * verification process.
 *
 * @param [in] expected_list_choice     Correct choice value as indicated by the application
 *
 ...
 *
 * @see wallet, MAX_NUMBER_OF_MNEMONIC_WORDS, MINIMUM_NO_OF_SHARES, TOTAL_NUMBER_OF_SHARES
 * @since v1.0.0
 *
 * @note
 */
...
/**
 * @brief Resets the value of particular properties global Flow_level instance based on the value passed
 * @details The function resets all the level members of flow_level for all the levels greater than specified
 * level.
 *
