...
/**
 * Dynamic metadata contains information such as offsets, number of bytes of data, etc.
 * Our assumption is that the value if exceeds by 4 bytes, device will not be able to decode it
 * in anyway as it will become out of bounds for the 32-bit MCU.
 * Therefore keeping ABI_DYN_METADATA_SZ_IN_BYTES as 4.
 */
 ...
 * only supports static Abi_Type_e as limited functionality is required currently.
 * 
 * @param inputAbiType Type of Abi data to be converted 
 * @param additionalInput This argument is optional:
 * ->if (inputAbiType == Abi_uint256_e or Abi_int256_e or Abi_bytes_e): This argument
 * informs about the number of bytes of relevant data is present.
