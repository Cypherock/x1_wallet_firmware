 ...
 * @details    To debug the line number and file of the project we can run gdb server with the elf file
 *          Open cmd and start gdb with executable name
 *          >> gdb Cypherock.elf
 *          (gdb) info line *<pc or lr value>
 *
 *          running this command returns file name, function and line number for that address
 *
...
#define ASSERT(expr) ((expr) ? (void)0U : assert_handler((uint8_t *)__FILE__, (uint8_t *)__FUNCTION__, __LINE__))
