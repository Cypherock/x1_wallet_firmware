...
typedef enum commandType {
  ...
  SEND_TXN_SENDING_SIGNED_TXN =
      54,  ///< Response by device for send transaction with signed transaction
  ...
  CONFIRM_PROVISION =
      80,  ///< Response by device(initial firmwaer) after provisioning complete/failed
  ...
  SEND_TXN_PRE_SIGNING_DATA =
      92,  ///< Command for data required just before signing in send transaction
  ...
  SIGN_MSG_SEND_SIG =
      95,  ///< Response by device for send transaction with signed transaction
 ...
 * @note
 */
void transmit_data_to_app(uint32_t command_type,
