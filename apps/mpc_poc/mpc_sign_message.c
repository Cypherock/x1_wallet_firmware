...
void compute_lambda(bignum256 *lambda, uint32_t *participant_indices, size_t participants_len, uint32_t index) {
  ...
  for (uint32_t i = 0; i < participants_len; i++) {
      int m = participant_indices[i];
      if (m == index)
          continue;

      num *= (int32_t)(m);
      den *= (int32_t)((int32_t) m - (int32_t) index);
  }
  ...
  LOG_INFO("NEW DUMP\n");  
  LOG_INFO("Dumping lambda value: %ld\n", num);
  LOG_INFO("Dumping participant indices: \n");
  for (int i = 0; i < participants_len; ++i) {
    LOG_INFO("%d\n", participant_indices[i]);
  }
  LOG_INFO("Dumping index: %d\n", index);
...
bool mta_send_rcv_pub_keys(mpc_poc_query_t *query,
  ...
  for (int i = 0; i < receiver_times; ++i) {
    ...
    stop_msg_display();
    display_msg_on_screen("Generating OT key pairs for the senders...");
...
bool mta_send_snd_pub_keys(mpc_poc_query_t *query,
  ...
  for (int i = 0; i < sender_times; ++i) {
    ...
    stop_msg_display();
    display_msg_on_screen("Generating OT key pairs for the receivers...");
    ...
    for (int j = 0; j < NUMBER_OF_OTS; ++j) {
      ...
      curve_point ot_receiver_pk;
      if (ecdsa_read_pubkey(curve, query->sign_message.mta_snd_get_pk.public_key, &ot_receiver_pk) == 0) {
        mpc_delay_scr_init("Error: reading pubkey failed", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
      }
    ...
    if (!verify_pub_key) {
      mpc_delay_scr_init("Error: malloc failed for verify_pub_key", DELAY_TIME);
      LOG_SWV("Error: malloc failed for verify_pub_key\n");
      mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                     ERROR_DATA_FLOW_INVALID_REQUEST);
      return false;
    }
...
bool mta_send_rcv_enc(mpc_poc_query_t *query,
  ...
  for (int i = 0; i < receiver_times; ++i) {
    ...
    stop_msg_display();
    display_msg_on_screen("Encrypting vertical IKNP matrices for the senders...");

    mpc_poc_result_t result =
        init_mpc_result(MPC_POC_RESULT_SIGN_MESSAGE_TAG);

    result.sign_message.which_response = MPC_POC_SIGN_MESSAGE_RESPONSE_MTA_RCV_GET_ENC_INITIATE_TAG;

    result.sign_message.mta_rcv_get_enc_initiate.to = participant_indices[i];
    result.sign_message.mta_rcv_get_enc_initiate.length = NUMBER_OF_OTS;
    ...
    if (!t_value || !ot_receiver_sk || !ot_receiver_pk || !ot_sender_pk || 
        !k0 || !k1 || !ot_receiver_pk_neg || !key_hash || !key_coordinate) {
      mpc_delay_scr_init("Error: malloc failed at the above of rcv enc function", DELAY_TIME);
      LOG_SWV("Error: malloc failed at the above of rcv enc function\n");
      mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                     ERROR_DATA_FLOW_INVALID_REQUEST);
      return false;
    }
    ...
    for (int j = 0; j < NUMBER_OF_OTS; ++j) {
      ...
      if (ecdsa_read_pubkey(curve, query->sign_message.mta_rcv_get_enc.public_key, ot_sender_pk) == 0) {
        mpc_delay_scr_init("Error: reading pubkey failed", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        free(t_value);
      ...
      if (!hasher_key0) {
        mpc_delay_scr_init("Error: malloc failed for hasher_key0", DELAY_TIME);
        LOG_SWV("Error: malloc failed for hasher_key0\n");
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
      }
      ...
      if (!hasher_key1) {
        mpc_delay_scr_init("Error: malloc failed for hasher_key1", DELAY_TIME);
        LOG_SWV("Error: malloc failed for hasher_key1\n");
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
      }
      ...
      hasher_Update(&hasher, result.sign_message.mta_rcv_get_enc.enc_m0, 128);
      hasher_Update(&hasher, result.sign_message.mta_rcv_get_enc.enc_m1, 128);
    ...
    result.sign_message.which_response = MPC_POC_SIGN_MESSAGE_RESPONSE_MTA_RCV_GET_ENC_SIG_TAG;

    uint8_t *hash = malloc(32 * sizeof(uint8_t));
    uint8_t *hash_verify = malloc(32 * sizeof(uint8_t));

    if (!hash || !hash_verify) {
      mpc_delay_scr_init("Error: malloc failed for hash or hash_verify", DELAY_TIME);
      LOG_SWV("Error: malloc failed for hash or hash_verify\n");
      mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                     ERROR_DATA_FLOW_INVALID_REQUEST);
      return false;
    }
    ...
    if (mpc_sign_message(hash, 32, result.sign_message.mta_rcv_get_enc_sig.signature, priv_key) != 0) {
      mpc_delay_scr_init("Error: signing failed", DELAY_TIME);
      free(hash);
      return false;
    }
...
bool mta_post_snd_enc(mpc_poc_query_t *query,
  ...
  for (int i = 0; i < sender_times; ++i) {
    ...
    stop_msg_display();
    display_msg_on_screen("Decrypting vertical IKNP matrices...");
    ...
    if (!dec_m || !dec_key || !ot_sender_sk || !ot_receiver_pk || !kc || !key_coordinate) {
      mpc_delay_scr_init("Error: malloc failed at the above of post snd enc function", DELAY_TIME);
      LOG_SWV("Error: malloc failed at the above of post snd enc function\n");
      mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                     ERROR_DATA_FLOW_INVALID_REQUEST);
      return false;
    }
    ...
    for (int j = 0; j < NUMBER_OF_OTS; ++j) {
      ...
      bn_read_be(&ot_sender_sk_lists[i * NUMBER_OF_OTS * 32 + j * 32], ot_sender_sk);
      if (ecdsa_read_pubkey(curve, &ot_sender_received_pks[i * NUMBER_OF_OTS * 33 + j * 33], ot_receiver_pk) == 0) {
        mpc_delay_scr_init("Error: reading pubkey failed", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        free(dec_m);
      ...
      if (!hasher_key) {
        mpc_delay_scr_init("Error: malloc failed for hasher_key", DELAY_TIME);
        LOG_SWV("Error: malloc failed for hasher_key\n");
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
      }
...
bool mta_mascot_snd(mpc_poc_query_t *query,
  ...
  for (int i = 0; i < sender_times; ++i) {
    ...
    stop_msg_display();
    display_msg_on_screen("Performing sender-side MASCOT using symmetric key operations...");
    ...
    for (int j = 0; j < 1024; ++j) {
      ...
      if (!q_value) {
        mpc_delay_scr_init("Error: malloc failed for q_value", DELAY_TIME);
        LOG_SWV("Error: malloc failed for q_value\n");
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
      }
      ...
      if (!rand_value) {
        mpc_delay_scr_init("Error: malloc failed for rand_value", DELAY_TIME);
        LOG_SWV("Error: malloc failed for rand_value\n");
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        free(q_value);
        return false;
      ...
      bn_fast_mod(&rand_bn, &curve->order);
      ...
      if (polynomial < 3) {
        bn_copy(&secret_share_list[ZERO_POLYNOMIALS + polynomial], &share_bn);
      } else {
        bn_read_be(priv_key_share, &share_bn);
      }

      if (j < 4) {
        LOG_INFO("DUMPING DUMPING\n");
        LOG_INFO("j: %d\n", j);
        LOG_INFO("enc_m0: ");
        char msg[65];
        for (int kp = 0; kp < 32; ++kp) {
          sprintf(msg + kp * 2, "%02x", rand_value[kp]);
        }
        msg[64] = '\0';
        LOG_INFO("%s\n", msg);
      }
...
bool mta_mascot_rcv(mpc_poc_query_t *query,
  ...
  for (int i = 0; i < receiver_times; ++i) {
    ...
    stop_msg_display();
    display_msg_on_screen("Performing receiver-side MASCOT using symmetric key operations...");
    ...
    for (int j = 0; j < 1024; ++j) {
      ...
      if (!dec_m) {
        mpc_delay_scr_init("Error: malloc failed for dec_m", DELAY_TIME);
        LOG_SWV("Error: malloc failed for dec_m\n");
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
      }
...
void reverse_bits(uint8_t *array, size_t size) {
    for (size_t i = 0; i < size; ++i) {
        uint8_t currentByte = array[i];
        uint8_t reversedByte = 0;

        for (int j = 0; j < 8; ++j) {
            reversedByte |= ((currentByte >> j) & 1) << (7 - j);
        }
...
bool start_mta(mpc_poc_query_t *query, 
  ...
  if (sender_times > 0) {
    ...
    if (!s_values) {
        mpc_delay_scr_init("Memory allocation failed for s_values", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                      ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
    }
    ...
    if (!ot_sender_sk_lists) {
        mpc_delay_scr_init("Memory allocation failed for ot_sender_sk_lists", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                      ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
    }
    ...
    if (!ot_sender_received_pks) {
        mpc_delay_scr_init("Memory allocation failed for ot_sender_received_pks", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                      ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
    }
  ...
  if (receiver_times > 0) {
    ...
    if (!b_value) {
        mpc_delay_scr_init("Memory allocation failed for b_value", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                      ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
    }
    ...
    bn_write_le(&secret_share_list[POLYNOMIAL_K_INDEX], b_value_ptr);
    b_value_ptr += 32;
    bn_write_le(&secret_share_list[POLYNOMIAL_A_INDEX], b_value_ptr);
    b_value_ptr += 32;

    bignum256 receiver_priv_key_share_bn;
    bn_read_be(priv_key_share, &receiver_priv_key_share_bn);

    bn_write_le(&receiver_priv_key_share_bn, b_value_ptr);
    b_value_ptr += 32;
    bn_write_le(&secret_share_list[POLYNOMIAL_P_INDEX], b_value_ptr);

    reverse_bits(b_value, 128);
    ...
    if (!ot_receiver_sk_lists) {
        mpc_delay_scr_init("Memory allocation failed for ot_receiver_sk_lists", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                      ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
    }
  ...
  if (receiver_times > 0 && !mta_send_rcv_pub_keys(query, participant_indices, receiver_times, 
                             priv_key, ot_receiver_sk_lists)) {
    return false;
  }

  if (sender_times > 0 && !mta_send_snd_pub_keys(query, group_info, index, 
                             participant_indices, sender_times, priv_key, s_values, 
                             ot_sender_sk_lists, ot_sender_received_pks)) {
    return false;
  }

  if (receiver_times > 0 && !mta_send_rcv_enc(query, group_info, index, participant_indices, receiver_times, 
                        priv_key, ot_receiver_sk_lists, b_value, t_matrices)) {
    return false;
  }

  // dump b_value and t_matrix
  if (receiver_times > 0) {
    LOG_INFO("Dumping b value very new\n");
    for (int j = 0; j < 4; ++j) {
      char msg[65];
      for (int i = 0; i < 32; ++i) {
    ...
    LOG_INFO("DDNEW Dumping first 256 bytes of t_matrix\n");
    for (int j = 256; j < 272; ++j) {
      char msg[33];
      for (int i = 0; i < 16; ++i) {
        sprintf(msg + i * 2, "%02x", t_matrices[0][j][i]);
      }
      msg[32] = '\0';
      LOG_INFO("%s\n", msg);
    }
    LOG_INFO("DUMPED b value and t_matrix\n");

    LOG_INFO("DDNEW Dumping second phase 256 bytes of t_matrix\n");
    for (int j = 0; j < 16; ++j) {
      char msg[33];
      for (int i = 0; i < 16; ++i) {
        sprintf(msg + i * 2, "%02x", t_matrices[0][j][i]);
  ...
  // dump s_values and q_matrix
  if (sender_times > 0) {
    LOG_INFO("Dumping s values very new\n");
    ...
    LOG_INFO("DDNEW Dumping first 256 bytes of q_matrix\n");
    for (int j = 256; j < 272; ++j) {
      char msg[33];
      for (int i = 0; i < 16; ++i) {
        sprintf(msg + i * 2, "%02x", q_matrices[0][j][i]);
      }
      msg[32] = '\0';
      LOG_INFO("%s\n", msg);
    }
    LOG_INFO("DUMPED s value and q_matrix\n");
    
    LOG_INFO("Dumping second phase 256 bytes of q_matrix\n");
    for (int j = 0; j < 16; ++j) {
      char msg[33];
      for (int i = 0; i < 16; ++i) {
        sprintf(msg + i * 2, "%02x", q_matrices[0][j][i]);
  ...
  if (sender_times > 0) {
    ...
    if (!sender_additive_shares_list) {
        mpc_delay_scr_init("Memory allocation failed for sender_additive_shares_list", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                      ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
    }
  ...
  if (receiver_times > 0) {
    ...
    if (!receiver_additive_shares_list) {
        mpc_delay_scr_init("Memory allocation failed for receiver_additive_shares_list", DELAY_TIME);
        mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                      ERROR_DATA_FLOW_INVALID_REQUEST);
        return false;
    }
  ...
  if (receiver_times > 0 && !mta_mascot_rcv(query, group_info, index, participant_indices, receiver_times,
...
bool sig_get_authenticator(mpc_poc_query_t *query,
  ...
  curve_point r_value;
  if (ecdsa_read_pubkey(curve, group_key_info->group_pub_key, &r_value) == 0) {
    mpc_delay_scr_init("Error: reading pubkey failed", DELAY_TIME);
    mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
    return false;
...
bool sig_compute_authenticator(mpc_poc_query_t *query,
  ...
  if (ecdsa_read_pubkey(curve, my_big_w_share, &my_point) == 0) {
    mpc_delay_scr_init("Error: reading pubkey failed", DELAY_TIME);
    mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
    return false;
  ...
  for (int i = 0; i < length; ++i) {
    ...
    curve_point point;
    if (ecdsa_read_pubkey(curve, signed_authenticator_data.authenticator_data.big_w_share, &point) == 0) {
      mpc_delay_scr_init("Error: reading pubkey failed", DELAY_TIME);
      mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
      return false;
...
bool sig_get_ka_share(mpc_poc_query_t *query,
  ...
  bn_multiply(a_share, k_inv_share, &curve->order);

  bn_copy(k_inv_share, &ka_share);
  bn_addmod(&ka_share, p_share, &curve->order);
  bn_mod(&ka_share, &curve->order);

  bignum256 lambda;
  compute_lambda(&lambda, participant_indices, threshold, participant_indices[index]);
...
void sign_message_flow(mpc_poc_query_t *query) {
  if (MPC_POC_SIGN_MESSAGE_REQUEST_INITIATE_TAG !=
    ...
    stop_msg_display();
    display_msg_on_screen("Getting things ready...");
    ...
    bignum256 my_ka_share;
    if (!sig_get_ka_share(query, &w, &secret_share_list[POLYNOMIAL_P_INDEX], 
                          &secret_share_list[POLYNOMIAL_A_INDEX], priv_key, &my_ka_share,
    ...
    const ecdsa_curve* curve = get_curve_by_name(SECP256K1_NAME)->params;
    if (ecdsa_read_pubkey(curve, group_key_info_list[POLYNOMIAL_K_INDEX].group_pub_key, &R) == 0) {
      mpc_delay_scr_init("Error: reading pubkey failed", DELAY_TIME);
      mpc_send_error(ERROR_COMMON_ERROR_CORRUPT_DATA_TAG,
                       ERROR_DATA_FLOW_INVALID_REQUEST);
      return false;
