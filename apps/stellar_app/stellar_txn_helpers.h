...
/**
 * @brief Parse XDR byte array of unsigned transaction and store decoded information to be
 * used for user confirmation.
 * @details Supports Stellar payment and create account transactions.
 *
 * @param xdr Constant reference to buffer containing the raw unsigned XDR transaction
 * @param xdr_len Size in bytes of the XDR transaction
 * @param tx Reference to buffer where decoded transaction information will be populated.
 * It can be used at a later stage for user verification.
 * @param payment Reference to buffer where decoded payment/create account operation will be populated.
 * @return int 0 if the parsing was successful, negative value if parsing failed
 * @retval 0 If the parsing was successful  
 * @retval -1 If the parsing failed - could be due to unsupported transaction, data type, or missing information
 */
