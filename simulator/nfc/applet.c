...
static uint8_t applet_select_apdu[] = {0x00, 0xa4, 0x04, 0x00, 0x05, 0x01, 0x02, 0x03, 0x04, 0x05};
static uint8_t applet_select_resp[] = {0xb0, 0x06, 0x01, 0x04, 0x01, 0x02, 0x03, 0x04, 0xb1, 0x05, 0xa1, 0xa2, 0xa3, 0xa4, 0x00};
...
ret_code_t applet_read(uint8_t* buffer, uint8_t size)
{
    ...
    if (!file) return STM_ERROR_INTERNAL;
    errno = 0;
...
static void process()
{
    ...
    switch (buffer[APDU_BASE_OFFSET + OFFSET_INS]) {
        case 0xa4:          // applet select
            if (memcmp(buffer + APDU_BASE_OFFSET + OFFSET_CLA, applet_select_apdu, sizeof(applet_select_apdu)) != 0) {
                off = 0;
                out_buffer[off++] = 0x69;
                out_buffer[off++] = 0x83;
                break;
            ...
            out_buffer[off - 1] = card_number; // put correct number as this program cycles b/w 1-4 cards
            out_buffer[off++] = 0x90;
            out_buffer[off++] = 0x00;
            break;
    ...
    for (int i = 0; i < MAX_WALLETS; i++) {
        ...
        if (curr_wallet->is_set) {
            if (memcmp(wallet->name, curr_wallet->name, sizeof(wallet->name)) == 0)
                return 0x6a80;
            if (memcmp(wallet->wallet_id, curr_wallet->wallet_id, sizeof(wallet->wallet_id)) == 0)
                return 0x6a80;
        }
    }
    wallet->is_set = 1;
...
{ for (int i = 0; i < 33; i++) printf("%02X ", buffer[i]); printf("\n");
    for (int i = 0; i < MAX_WALLETS; i++) {
        ...
        if (wallet->is_set && memcmp(buffer + 7, wallet->name, sizeof(wallet->name)) == 0) {
            if (WALLET_IS_PIN_SET(wallet->wallet_info))
                if (memcmp(buffer + 25, wallet->password, sizeof(wallet->password)) == 0)
                    return i;
                else {
