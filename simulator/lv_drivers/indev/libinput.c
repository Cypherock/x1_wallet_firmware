...
bool libinput_set_file(char *dev_name) {
  ...
  // handles. citing libinput.h:libinput_path_remove_device: > If no matching
  // device exists, this function does nothing.
  if (libinput_device) {
  ...
  while ((event = libinput_get_event(libinput_context)) != NULL) {
    ...
    switch (type) {
      ...
      case LIBINPUT_EVENT_TOUCH_DOWN:
        ...
        most_recent_touch_point.x =
            libinput_event_touch_get_x_transformed(touch_event, LV_HOR_RES);
        most_recent_touch_point.y =
            libinput_event_touch_get_y_transformed(touch_event, LV_VER_RES);
