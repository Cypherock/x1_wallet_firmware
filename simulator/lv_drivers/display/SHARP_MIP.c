 ...
 *
 *  Since extra bytes (dummy, addresses, header) are stored in VDB, we need to
 *use an "oversized" VDB. Buffer declaration in "lv_port_disp.c" becomes for
 *example : static lv_disp_buf_t disp_buf; static uint8_t buf[(LV_VER_RES_MAX /
 *X) * (2 + (LV_HOR_RES_MAX / 8)) + 2]; lv_disp_buf_init(&disp_buf, buf, NULL,
 *LV_VER_RES_MAX * LV_HOR_RES_MAX / X);
 *-----------------------------------------------------------------------------------------------*/
...
void sharp_mip_flush(lv_disp_drv_t *disp_drv,
  ...
  uint16_t act_y2 =
      area->y2 > SHARP_MIP_VER_RES - 1 ? SHARP_MIP_VER_RES - 1 : area->y2;

  uint8_t *buf = (uint8_t *)color_p;      /*Get the buffer address*/
  uint16_t buf_h = (act_y2 - act_y1 + 1); /*Number of buffer lines*/
  uint16_t buf_size =
      buf_h * (2 + SHARP_MIP_HOR_RES / 8) + 2; /*Buffer size in bytes  */
...
void sharp_mip_set_px(lv_disp_drv_t *disp_drv,
                      uint8_t *buf,
                      lv_coord_t buf_w,
                      lv_coord_t x,
                      lv_coord_t y,
  ...
  if (lv_color_to1(color) != 0) {
    buf[BUFIDX(x, y)] |=
        PIXIDX(x); /*Set VDB pixel bit to 1 for other colors than BLACK*/
  } else {
