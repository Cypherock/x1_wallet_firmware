...
typedef struct {
  ...
  uint32_t fwSize;
  uint32_t
      auth_state;  ///< Device Authentication state; Interpreted as "whether device is authenticated or not?"; Indicated by ENUM device_auth_state (32-bit value)
  uint32_t
      first_boot_on_update;  ///< 0 - not first boot after update, 1 - first boot after update
