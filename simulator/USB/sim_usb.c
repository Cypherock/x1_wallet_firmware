...
#ifdef _WIN32
    snprintf(file_name, sizeof(file_name), "%s\\%s", getenv(TEMP_ENV_VAR), SIM_USB_RX_FILE_NAME);
#else
...
#ifdef _WIN32
    snprintf(file_name, sizeof(file_name), "%s\\%s", getenv(TEMP_ENV_VAR), SIM_USB_TX_FILE_NAME);
#else
...
static void check_for_usb_data()
{
    ...
    if (count == COMM_HEADER_SIZE) {
        count = fread(buffer + COMM_HEADER_SIZE, sizeof(uint8_t), buffer[DATA_SIZE_INDEX], file) + COMM_HEADER_SIZE;
        seek_pos += count;
        printf("%d\n", count);
